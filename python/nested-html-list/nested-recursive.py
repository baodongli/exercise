...
def generate(parent, headings):
    if not headings:
        return None
    h = headings[0]
    cur_weight = h.weight
    hs = []
    if cur_weight > parent.heading.weight:
        new_node = Node(h, [])
        parent.children.append(new_node)
        hs = generate(new_node, headings[1:])
    elif h.weight <= parent.heading.weight:
        return headings

    while hs and hs[0].weight == cur_weight:
        h = hs[0]
        new_node = Node(h, [])
        parent.children.append(new_node)
        hs = generate(new_node, hs[1:])
    return hs

node = Node(Heading(0, "My Article"), [])
generate(node, [h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11, h12])
PrintNode(node)
